...
| Layer | Choice |
|-------|--------|
| Runner | `bun test` with `--preload ./test/setup.ts` (no `--isolate`; see note below) |
| API | `bun:test` |
...
Bun 1.3.x can segfault when `--isolate` runs many test files in one invocation (e.g. `ThemePicker` + `FormGroup` together). Per-file isolation still works: `bun test --preload ./test/setup.ts --isolate path/to/one.test.tsx`. The default `bun run test` omits `--isolate` so the full suite completes reliably; mocks are registered once in `setup.ts` and tests should clear shared mocks in `beforeEach` where needed.
